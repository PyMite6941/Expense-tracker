...
| Version | Date | Notes |
|---------|------|-------|
| v1.6 | September 2026 | Auth hardening: secrets fail closed, `/issue` no longer public when `ISSUE_SECRET` is unset, `X-Forwarded-For` read from the right so the device cap is enforceable, licence keys no longer written to logs, `.gitignore` names secrets explicitly. React (`web/`) client; Streamlit dashboard reworked with a month selector |

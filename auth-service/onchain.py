"""
Verify USDC-on-Base payments on-chain via a public Base RPC (no API key).
...
Used by the public /redeem endpoint so buyers can self-serve their license
the moment their payment lands — no manual step.
"""
import logging
import os
...
# Store wallet that must receive the payment.
PAYMENT_WALLET = os.getenv(
    "PAYMENT_WALLET", "0x8069408a17B77895cb7Cd0B0D804aB46f59Bc4c3"
).lower()

# Accepted USDC token contracts on Base (native USDC + bridged USDbC), 6 decimals.
_DEFAULT_TOKENS = (
...
def _addr_from_topic(topic: str) -> str:
    # A 32-byte topic holds a left-padded address; the address is the last 40 hex chars.
...
def verify_usdc_payment(tx_hash: str, min_usdc: float):
    ...
    ok is True only if tx_hash is a confirmed, successful USDC transfer to the
    store wallet totalling at least min_usdc.
    """
    ...
    # Sum USDC Transfer events in this tx that credit the store wallet.
    smallest = 0
    ...
    # Confirmations (don't hard-fail the whole check on a transient blockNumber error).
    try:
